...
popFiles <- list.files('pyrho_1kgp_chm13_unphased/chrX_nPAR/opt_params_masked_masked')

for (f in popFiles){
  pop <-  strsplit(f, '_')[[1]][1]
  
  # Get the list of optimized parameters for this population
  optimized_params <- read.table(paste0('pyrho_1kgp_chm13_unphased/chrX_nPAR/optimize_paramaters/',
  ...
  in_name <- paste0('pyrho_1kgp_chm13_unphased/chrX_nPAR/opt_params_masked_masked/', f)
  ...
  out_name <- paste0('pyrho_1kgp_chm13_unphased/chrX_nPAR/opt_params_toAvg/', pop, '_chrX_nPAR.tsv')

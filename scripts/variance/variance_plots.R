...
all_maps <- data.frame(matrix(ncol=6,nrow=0, 
                              dimnames=list(NULL, c("Chr", "Pop", "Start", "End", "cM_in_window", "cM_scaled_in_window"))))
...
split_scaled_map <- function(pyrho_map, window_size){
  ...
  out_map <- data.frame(matrix(ncol = 6, nrow = 0))
  colnames(out_map) <- c("Chr", "Pop", "Start", "End", "cM_in_window", "cM_scaled_in_window")
  ...
  for (i in 0:n_windows){
    ...
    if (nrow(pyrho_map_in_interval) > 0){
      cM_in_window <- sum(pyrho_map_in_interval$cM)
      cM_scaled_in_window <- sum(pyrho_map_in_interval$cM.Scaled)
      
      out_map[nrow(out_map) + 1, ] <- c(paste0("chr", chromosome), pop, interval_start, interval_stop, cM_in_window, cM_scaled_in_window)
    }
  }
...
for (pop in pops){
  ...
  in_name <- paste0("../../pyrho_1kgp_chm13_unphased_no_mask/opt_params/", list.files("../../pyrho_1kgp_chm13_unphased_no_mask/opt_params/")[grep(fname_prefix, list.files("../../pyrho_1kgp_chm13_unphased_no_mask/opt_params/"))])
...
write_csv(out_df,
          paste0("variance_tables/chr", chromosome, ".csv"))
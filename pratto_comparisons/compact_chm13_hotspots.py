...
for pop in pops:
	...
	for thresh in [5, 10, 50, 100]:
		# Data import
		in_name = "hotspots/chm13/" + str(thresh) + "x/" + pop + "_" + str(thresh) + "x_hotspots.csv"
		in_file = pd.read_csv(in_name)
	  
		in_file['Chromosome'] = in_file['Chromosome'].astype(int)
		...
		for i in range(in_file.shape[0]):
			if i == 0:
				...
				rec_rates.append(in_file['Rec.Rate.Scaled'][i])
	  
			else:
				if in_file['Start'][i] == end_coords[-1]:
					end_coords[-1] = in_file['End'][i]
					rec_rates[-1] = max(rec_rates[-1], in_file['Rec.Rate.Scaled'][i])
				else:
					chromosomes.append(in_file['Chromosome'][i])
					start_coords.append(in_file['Start'][i])
					end_coords.append(in_file['End'][i])
		...
		out_df = pd.DataFrame({'Chromosome': chromosomes, 
			'Start': start_coords, 
			'End': end_coords, 
			'Rec.Rate.Scaled': rec_rates})	

		out_df['Chromosome'] = 'chr' + out_df['Chromosome'].astype(str)
			
		out_name = "hotspots_compact/chm13/" + str(thresh) + "x/" + pop + "_" + str(thresh) + "x_hotspots_compact.bed"

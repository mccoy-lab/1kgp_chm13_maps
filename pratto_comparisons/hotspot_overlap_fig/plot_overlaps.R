...
#   geom_text(data = row_anno,
#             aes(x = row, y = 0, label = row, color = group),
#             inherit.aes = FALSE, vjust = 0, hjust = 0.8, angle = 90,
#             show.legend = FALSE) +
#   geom_text(data = row_anno,
#             aes(x = 0, y = row, label = row, color = group),
#             inherit.aes = FALSE, vjust = 0.3, hjust = 0.8, angle = 0,
#             show.legend = FALSE)
# 
# g <- ggplot_build(a)
# row_colors <- g$data[[2]]$colour 
# 
# a <- a +
#   scale_color_manual(
#     name = "Group",
#     values = row_colors,
#     guide = guide_legend(override.aes = list(label = NA, shape = 15, size = 5))
#   )

ggsave("hotspot_overlap.png",
       dpi = 300)
